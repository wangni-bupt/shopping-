...
public class SortServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		else if("updateSort".equals(method)){
			Sort sort = new Sort();
			sort.setSortId(Integer.parseInt(request.getParameter("sortId")));
			sort.setSortName(request.getParameter("sortName"));
			flag = sortservice.updateSort(sort);
			result = sortservice.getResult();
			if(flag){
				response.sendRedirect("SortServlet.do?method=selectSort");
			}else{
				...
				request.getRequestDispatcher(
						"background/bg-sortUpdate.jsp?sortId="
								+ sort.getSortId() + "&sortName" + sort.getSortName()
								).forward(request, response);
			}
		}
	}
