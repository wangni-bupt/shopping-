...
List<Goods> freeList = (List<Goods>) request.getAttribute("list");
		...
		<div id="content">
			...
			<div class="goodsTwo">
				...
				<div class="goodsTwoTextarea1">
					简介：<%=freeGoods.getIntroduce()%>
				</div>
				<div class="goodsTwoTextarea2">
					<p><%=freeGoods.getBookName()%></p>
					<%
						if(freeGoods.getMark()==0){
					%>
					<p class="blue">
						现价：<%=freeGoods.getNowprice()%>元
					</p>
					<%
						}else{
					%>
					<p class="red" style="text-decoration:line-through;">
						原价：<%=freeGoods.getNowprice()%>元
					</p>
					<p class="blue">
						现价：<%=freeGoods.getFreeprice()%>元
					</p>
					<%
						}
					%>
					...
					<p style="line-height:90px">
						<a href="GoodsServlet.do?method=selectGoodsByGoodsId&goodsId=<%=freeGoods.getGoodsId()%>&sign=fg">查看详细内容</a>
					</p>
					<%
						}else{
					%>
			...
			<div class="page">
				...
				<div class="pageOne">
					<a
						href="GoodsServlet.do?method=selectGoodsByMark&mark=1&i=<%=number-1%>">上一页</a>
				</div>
				<%
					}
				%>
				...
				%>
				<div class="pageOne">
					<a href="GoodsServlet.do?method=selectGoodsByMark&mark=1&i=<%=number+1%>">下一页</a>
				</div>
				<%
					}
				%>
