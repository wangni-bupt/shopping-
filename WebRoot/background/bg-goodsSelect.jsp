				...
				<table class="tableBorder" style="width:99%;">
						...
						<tr style="background-color:#FFFFFF">
							...
							<td>
								...
								%>
								<%System.out.println(goods.getFirst()); %>
							</td>
							<td><a href="GoodsServlet.do?method=selectGoodsByGoodsId&goodsId=<%=goods.getGoodsId()%>&sign=bg">详细信息</a>&nbsp;
								<a href="javascript:deleteGood('<%=goods.getGoodsId()%>')">删</a>
								<a href="background/bg-goodsNameUpdate.jsp?goodsId=<%=goods.getGoodsId()%>">修改</a>
								<%if(goods.getFirst()==1){%>
									 已置顶
		 						<%
									}else{
								%>
		  						<a href="GoodsServlet.do?method=selectGoodsFirst&goodsId=<%=goods.getGoodsId()%>">置顶</a>
		 						<%}%>								
							</td>
						</tr>
						<%
